...
def draw_picture(results=None, details=None, portfolio_scale=0, end_date=date.today(), fun='', fund_pool_fun=''):
    ...
    plt.savefig(os.getcwd() + "\\vill\\results\\portfolio_" + end_date.__str__() + ".png",
                format='png', bbox_inches='tight')
    plt.savefig(os.getcwd() + "\\vill\\snapshots\\portfolio_" + end_date.__str__() + ".png",
                format='png', bbox_inches='tight')
    #plt.show()  # 让图形显现
    # plt.close()
...
if __name__ == '__main__':
    df = pd.read_csv(os.getcwd() + "\\vill\\results\\fund_nav_rate.csv", index_col=0)  # 读取基金收益率数据
    df = df.iloc[:, 0:10]
