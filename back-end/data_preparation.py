...
def get_fund_pool(fund_pool_update=False, update_date=date.today(), return_detail=False, 
    ...
    if not fund_pool_update:
        ...
        # 读取候选基金池
        try:
            res = pd.read_csv(os.getcwd() + "\\vill\\snapshots\\fundpool_" + fund_pool_fun + "_" + update_date.__str__() + ".csv", index_col=0, header=None, dtype=object)
    ...
    # 存储候选池基金全部信息
    res.to_csv(os.getcwd() + "\\vill\\results\\fundpool_" + fund_pool_fun + ".csv", index=True, encoding="utf_8_sig")
    res.to_csv(os.getcwd() + "\\vill\\snapshots\\fundpool_" + fund_pool_fun + "_" + update_date.__str__() + ".csv", index=True,
              encoding="utf_8_sig")
...
def get_nav_rates(fund_nav_update=False, fund_pool_update=False, begin_date="2019-10-17",
    ...
    if not fund_nav_update:
        ...
        # 读取候选基金池
        fund_nav_rates = pd.read_csv(os.getcwd() + "\\vill\\snapshots\\fund_nav_rate_" + fund_pool_fun + "_" + end_date.__str__() + ".csv", index_col=0, header=None, dtype=object)  # 读取基金收益率数据
    ...
    # 净值信息进行存储
    pd.DataFrame.to_csv(pd.DataFrame(fund_nav_rates), os.getcwd() + "\\vill\\results\\fund_nav_rate_" + fund_pool_fun + ".csv", index=True)
    pd.DataFrame.to_csv(pd.DataFrame(fund_nav_rates),
                        os.getcwd() + "\\vill\\snapshots\\fund_nav_rate_" + fund_pool_fun + "_" + end_date.__str__() + ".csv", index=True)
    # 读取基金收益率数据
...
def get_previous_date(interval, end_date):
    previous_dates = []
    for ind in interval:
        tmp_date = end_date - timedelta(ind)
        #print(tmp_date)
