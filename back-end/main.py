...
def buy(buy_w, w, data, date, money, asset, rate = 0, rate_cost = 0):
    hold = 0
    #print(buy_w)
    ...
    for code in buy_w.index:
        #close = float(finance.run_query(query(finance.FUND_NET_VALUE).filter(finance.FUND_NET_VALUE.code==code,finance.FUND_NET_VALUE.day == date))['refactor_net_value'])
        close = data.loc[date, code]
        wb = money * (1-rate) * float(buy_w.loc[code]) / close
        if w.get(code):
            w[code] += wb
        else:
            w[code] = wb
        cost = close * wb * (1 + rate)
        money -= cost
    ...
    #print(date, asset, hold, money, cost, w)
    return w, money, tmp, rate_cost
...
def draw_profits(a, c, r, cor1, cor2, filename,profit=0,maxloss=0, dates = []):    #zym 画折线图
...
def draw_w(buy_w, filename):                 #zym 画饼图
    plt.rcParams['figure.figsize'] = (20, 10)
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来正常显示中文标签
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    ...
    labels = list(buy_w.index)  # 标签，即种类        #zym 基金种类
    sizes = list(buy_w.iloc[:, 0] * 1000)  # 比例       #zym 基金所占比例 这里为什么是×1000
    ...
    pct = [str(x) + "%" for x in sizes]        #zym
    labels = [labels[i] + "-" + pct[i] for i in range(len(pct))]      #zym 饼图输出
...
def fun():
    ...
    pmonth = [6, 12]
    fun = ['md', 'rmd']
    gd = ['高风险', '中风险']             # zym 想法很简单！ 
    for root, dirs, files in os.walk('input\\'):
        for z in range(0,len(files),2):     
            filename = files[z]
            sellfilename = files[z+1]
            buydata = pd.read_excel('input\\'+filename, dtype=str)
            #buydata = buydata[:-1]
            ...
            for index in buydata.index:
                ...
                buydata.loc[index, '买入时间'] = t
            buydata.index = pd.to_datetime(buydata['买入时间'])

            selldata = pd.read_excel('input\\'+sellfilename, dtype=str)
            if not selldata.empty:
                ...
                for index in selldata.index:
                    ...
                    selldata.loc[index, '赎回时间'] = t
                selldata.index = pd.to_datetime(selldata['赎回时间'])

            today = datetime.today()
            sd = buydata.index[0]     #zym date sd
            ...
            for date in dates:
                flag = True
                if i < len(buydata) and buydata.index[i] == date:
                    ...
                    flag = False
                if s < len(selldata) and selldata.index[s] == date:
                    ...
                    if tmpdata.index[0]=='基金代码':
                        ...
                        for j in range(len(tmpdata)):
                            sell_w[tmpdata.iloc[j, 0]] = float(tmpdata.iloc[j, 2])
                        s += len(tmpdata)
                    rate = 0.005
            ...
            for d in [0, 1]:                # zym 行吧！
                ...
                for date in dates:
                    ...
                    if date in buydata.index:
                        ...
                        quarter = (date.month - 1) // 3 + 1
                        buy_w_filename = 'tmp\\'+str(date.year)+'_'+str(quarter)+ '_'+str(pmonth[d]) + 'limit_' + fun[d] +'.csv'
                        if os.path.exists(buy_w_filename):
                            ...
                            buy_w = pc.get_portfolio(money_scale=1e+7, up_limit_percent=0.5, fund_pool_size=100, fund_pool_update=True,
                                            ...
                                            fund_filename='funds\\'+str(date.year)+'_'+str(quarter)+'.txt')
                        ...
                        w, money, tmp, rate_cost = buy(buy_w, w, data, date, money, asset, rate, rate_cost)
                        #print('buy',date)
                        flag = False
                    ...
                    if date == end_date:
                        rate = 0.005
                        sell_w = w.copy()
                        w, money, tmp = sell(sell_w, w, data, date, money, asset, rate)
                        #print('sell',date)
                        flag = False
                ...
                fig2 = 'figure\\'+ filename[:-5] + gd[d] + '风险组合' + '.png'
                ...
                piename = 'figure\\' +filename[:-5] + gd[d] + '风险组合_pie.png'
                
                t_quarter = (today.month - 1) // 3 + 1
                t_date = get_quarter_date(today)
                buy_w_filename = 'tmp\\'+str(today.year)+'_'+str(t_quarter)+ '_'+str(pmonth[d]) + 'limit_' + fun[d] +'.csv'
                if os.path.exists(buy_w_filename):
                    buy_w = pd.read_csv(buy_w_filename, dtype=object)
                    buy_w.set_index('Unnamed: 0',inplace=True)
                else:
                    buy_w = pc.get_portfolio(money_scale=1e+7, up_limit_percent=0.5, fund_pool_size=100, fund_pool_update=True,
                                            ...
                                            fund_filename='funds\\'+str(today.year)+'_'+str(t_quarter)+'.txt')
                    buy_w.to_csv(buy_w_filename)
                ...
                if d == 0:
                    labels_high = list(buy_w.index)
                    names_high = namei
                    sizes_high = list(buy_w.weights)
                ...
                if d == 1:
                    labels_mid = list(buy_w.index)
                    names_mid = namei
                    sizes_mid = list(buy_w.weights)
            ...
            # ----------------------
            # difengxian
            low_limit = random.randint(int(80),int(90)) / 100
            w = {}
            cost = 0
            asset = 0
            money = 0
            ...
            for date in dates:
                ...
                if date in buydata.index:
                    ...
                    end_date = get_trade_days(end_date=start_dates[i] + timedelta(-1) , count=1)[0]
                    quarter = (date.month - 1) // 3 + 1
                    buy_w_filename = 'tmp\\'+str(date.year)+'_'+str(quarter)+ '_low.csv'
                    if os.path.exists(buy_w_filename):
                        buy_w = pd.read_csv(buy_w_filename, dtype=object)
                        buy_w.set_index('Unnamed: 0',inplace=True)
                    else:
                        buy_w = pc.get_portfolio(money_scale=1e+7, up_limit_percent=0.9, fund_pool_size=100, fund_pool_update=True,
                                        ...
                                        fund_filename='funds\\'+str(date.year)+'_'+str(quarter)+'.txt')
                        buy_w = revise_w(buy_w, 'funds\\'+str(today.year)+'_'+str(quarter)+'_low.txt', low_limit)
                        buy_w.to_csv(buy_w_filename)
                    tdata = get_fund_data(start_dates[0], start_dates[-1], buy_w)
                    if data.empty:
                        data = tdata
            ...
            value_list=list(value_df.value)
            result2,maxloss2 = bt.get_result(profit, 0.03,value_list)
            fig2 = 'figure\\'+ filename[:-5] + '低风险组合' + '.png'
            p_money2 = result2['r']*asset
            md_money2 = result2['md']*asset
            draw_profits(profit, profit1, result2, 'red', 'blue',fig2, p_money2, maxloss2, start_dates[:-1])
            
            piename = 'figure\\' +filename[:-5] + '低风险组合_pie.png'
            t_quarter = (today.month - 1) // 3 + 1
            t_date = get_quarter_date(today)
            buy_w_filename = 'tmp\\'+str(today.year)+'_'+str(t_quarter)+ '_low.csv'
            if os.path.exists(buy_w_filename):
                buy_w = pd.read_csv(buy_w_filename, dtype=object)
                buy_w.set_index('Unnamed: 0',inplace=True)
            else:
            ...
            #zym
            labels_low = list(buy_w.index)
            sizes_low = list(buy_w.weights)
            
    

    #print(profit1)

    #print(baseline_hs300)
    #print(baseline_hs300[0])
    #print(baseline_hs300[1])
    #print(len(baseline_hs300))  #baseline_hs300比profit1多一天 我感觉可以统一为profit1的日期
    #print(len(profit1))
    #print(baseline_hs300.index[1])
    #banfa = str(baseline_hs300.index[1])
    #banfa = banfa.replace(" 00:00:00","")
    #print(banfa)
    ...
    duration = [6,12,12] #按照目前的代码 高中低风险周期分别是6/12/12个月 此处等投资组合组写完可改
    ...
    return chart1, chartadd1, in1, duration, chart2_high, chart2_mid, chart2_low, chartadd2_high, chartadd2_mid, chartadd2_low, in2_high, in2_mid, in2_low, in3_high, in3_mid, in3_low
