...
def fun():
    ...
    for root, dirs, files in os.walk(os.path.dirname(__file__)+'\\input\\'):
        for z in range(0,len(files), 2):
            ...
            #ed = today.date()        # why 这里是why暂时改的 正常的时候改回去
            #-----------------
            ...
            figname2 = os.path.dirname(__file__)+'\\figure\\' +filename[:-5] + '.png'
            ...
            while date < today.date():
                ...
                date += dateutil.relativedelta.relativedelta(months=12)
            ...
            for date in dates:
                ...
                if date in buydata.index:
                    ...
                    quarter = (date.month - 1) // 3 + 1
                    buy_w_filename = os.path.dirname(__file__)+'\\tmp\\'+str(date.year)+'_'+str(quarter)+ '_low.csv'
            ...
            figname_l = os.path.dirname(__file__)+'\\figure\\' +filename[:-5] + '_low.png'
            ...
            piename = os.path.dirname(__file__)+'\\figure\\' +filename[:-5] + '_pie.png'
            ...
            for t in range(24):
                ...
                test_it = history_quarter[t]
                test_year = test_it.year
                test_quarter = (test_it.month -1) //3 +1
                test_date = get_quarter_date(test_it)
                buy_w_test_filename =os.path.dirname(__file__)+'\\'+ 'tmp\\'+str(test_year)+'_'+str(test_quarter)+ '_low.csv'
                ...
                pt_low = [test_year, test_quarter, labels_low, names_low, sizes_low] 
            ...
            buy_w_filename = os.path.dirname(__file__)+'\\tmp\\'+str(today.year)+'_'+str(t_quarter)+ '_low.csv'
            if os.path.exists(buy_w_filename):
                buy_w = pd.read_csv(buy_w_filename, dtype=object)
                buy_w.set_index('Unnamed: 0',inplace=True)
            else:
            ...
            wd.write_docx(filename[:-5], sd, '低风险','1年', figname2, p_per2, profit_f, figname_l, p_per_l, p_money_l, md_l, piename, buy_w)
            ...
            for date in dates:
                ...
                if date in buydata.index:
                    ...
                    end_date = get_trade_days(end_date=start_dates[i] + timedelta(-1) , count=1)[0]
                    quarter = (date.month - 1) // 3 + 1
                    buy_w_filename = os.path.dirname(__file__)+'\\tmp\\'+str(date.year)+'_'+str(quarter)+ '_'+str(pmonth[d]) + 'limit_' + fun[d] +'.csv'
                    if os.path.exists(buy_w_filename):
                        buy_w = pd.read_csv(buy_w_filename, dtype=object)
                        buy_w.set_index('Unnamed: 0',inplace=True)
                    else:
                ...
                if i+1 < len(start_dates) and date == start_dates[i]:
                    ...
                    if os.path.exists(buy_w_filename):
                        ...
                        buy_w = pc.get_portfolio(money_scale=1e+7, up_limit_percent=0.5, fund_pool_size=100, fund_pool_update=True,
                                         ...
                                         fund_filename=os.path.dirname(__file__)+'\\funds\\'+str(date.year)+'_'+str(quarter)+'.txt')
                        buy_w = revise_w_gao(buy_w, 0.11)
                        buy_w.to_csv(buy_w_filename)   #zym 这里的含义应该就是 没有现成的就算 算完了存起来
                    tdata = get_fund_data(start_dates[0], start_dates[-1], buy_w)
                    if data.empty:
                        data = tdata
                    else:
            ...
            for t in range(24):
                ...
                test_quarter = (test_it.month -1) //3 +1
                test_date = get_quarter_date(test_it)
                buy_w_test_filename =os.path.dirname(__file__)+'\\'+ 'tmp\\'+str(test_year)+'_'+str(test_quarter)+ '_'+str(pmonth[d]) + 'limit_' + fun[d] +'.csv'
                if os.path.exists(buy_w_test_filename):
                    buy_w_test = pd.read_csv(buy_w_test_filename, dtype=object)
                    buy_w_test.set_index('Unnamed: 0',inplace=True)
                else:
                ...
                pt_high = [test_year, test_quarter, labels_high, names_high, sizes_high] 
            ...
            t_quarter = (today.month - 1) // 3 + 1
            t_date = get_quarter_date(today)
            buy_w_filename = os.path.dirname(__file__)+'\\tmp\\'+str(today.year)+'_'+str(t_quarter)+ '_'+str(pmonth[d]) + 'limit_' + fun[d] +'.csv'
            if os.path.exists(buy_w_filename):
                buy_w = pd.read_csv(buy_w_filename, dtype=object)
                buy_w.set_index('Unnamed: 0',inplace=True)
            else:
            ...
            for t in range(24):
                ...
                pt_mid = [test_year, test_quarter, labels_mid, names_mid, sizes_mid] 
